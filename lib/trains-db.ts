...
export function searchLocalTrains(query: string): TrainEntry[] {
  ...
  return TRAINS_DB
    .map((train) => {
      ...
      const from = train.from.toLowerCase();
      const to = train.to.toLowerCase();
      const fromCode = train.fromCode.toLowerCase();
      const toCode = train.toCode.toLowerCase();
      ...
      if (isNumericQuery) {
        ...
        if (from.startsWith(q)) score = Math.max(score, 75);
        else if (from.includes(q)) score = Math.max(score, 55);
        if (to.startsWith(q)) score = Math.max(score, 75);
        else if (to.includes(q)) score = Math.max(score, 55);
        if (fromCode === q || toCode === q) score = Math.max(score, 60);
        else if (fromCode.startsWith(q) || toCode.startsWith(q)) score = Math.max(score, 50);
      }

      return { train, score };
    })
    .filter(({ score }) => score > 0)
    .sort((a, b) => b.score - a.score || a.train.number.localeCompare(b.train.number))
    .map(({ train }) => train)
    .filter((train) => {
      if (seen.has(train.number)) return false;
